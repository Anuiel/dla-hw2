...
from src.datasets.lip_dataset.lipreading.lipreading.dataloaders import (
...
def create_lipreading_index(config: LipReadingConfig) -> bool:
    assert (
        config.data_path.exists() and config.data_path.is_dir()
    ), f"No such directory {config.data_path}!"
    assert (
        config.model_config_path.exists() and config.model_config_path.is_file()
    ), f"No such file: {config.config_path}!"
    assert (
        config.model_path.exists() and config.model_path.is_file()
    ), f"No such file: {config.model_path}!"
...
def get_model_from_json(args: LipReadingConfig):
    assert str(args.model_config_path).endswith(".json") and os.path.isfile(
