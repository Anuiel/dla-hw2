...
class PESQ(BaseMetric):
    def __init__(self, device: str, mode: str, sampling_rate: int = 16000, *args, **kwargs):
        """
        Args:
            compute_on_cpu: compute si-snr of cpu or gpu
        """
        super().__init__(*args, **kwargs)
        self.pesq = PerceptualEvaluationSpeechQuality(sampling_rate, mode).to(device=device)
